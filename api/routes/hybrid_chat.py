...
from services.chat.hybrid_chat_service import (
    create_hybrid_message,
)
...
@router.post(
    "/{conversation_id}/hybrid-messages",
    response_model=ConversationMessageExchangeResponse,
    status_code=status.HTTP_201_CREATED,
    summary="Create a secure hybrid chat message",
)
async def create_hybrid_message_endpoint(
    ...
    try:
        exchange = await create_hybrid_message(
            session=session,
        ...
        raise HTTPException(
            ...
            detail="The hybrid chat message could not be saved.",
        ) from exc
